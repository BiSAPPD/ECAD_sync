    ...
    and 
        (Case lower(sln.name)
            when 'парикмахерская' then True 
            when 'салон красоты' Then True
            when 'салон'Then True 
            when 'магазин' Then True
            when 'студия красоты' Then True
            when 'проф магазин' Then True
            when 'ч/м' Then True
            when 'чм' Then True
        Else False
        end) is False Then 
        Count(Concat(sln.city_name_geographic, sln.house, sln.name)) over (Partition by Concat(sln.city_name_geographic, sln.street, sln.name))
    end) as dbl_status,
    ...
    group by sln.city_name_geographic, sln.street,  sln.name, sln.id, club
    --oRDER by  sln.city_name_geographic, sln.street
    order by sln.city_name_geographic, sln.street, sln.name, club, Manager desc , countUSR desc